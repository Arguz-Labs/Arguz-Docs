...
El bundle de agentes de Arguz es el paquete in-cluster soportado para conectar un cluster de Kubernetes con Arguz. El chart publico actual instala tres agentes:
...
2. El chart guarda esos valores en un Secret compartido y despliega los agentes en el namespace `arguz-agent`.
...
8. Node Agent corre en nodos worker Linux seleccionados y envia telemetria runtime observada a Arguz, donde esta disponible en los flujos de observabilidad de `app.arguz.io`.
...
| [Node Agent](node-agent.md) | Captura de telemetria runtime, requisitos de host y como explorarla en `app.arguz.io` |
