...
The Arguz agent bundle is the supported in-cluster package used to connect a Kubernetes cluster with Arguz. The current public chart installs three agents:
...
2. The Helm chart stores those values in a shared Kubernetes Secret and deploys the agents in the `arguz-agent` namespace.
...
8. Node Agent runs on selected Linux worker nodes and sends observed runtime telemetry to Arguz, where it is available through the observability workflows in `app.arguz.io`.
...
| [Node Agent](node-agent.md) | Runtime telemetry capture, host requirements and how to explore it in `app.arguz.io` |
